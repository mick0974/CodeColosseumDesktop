...
export class FormControlValidationMsgDirective implements OnInit, OnDestroy{
    ...
    statutsChangeSubscription: Subscription;

    ngOnInit(): void {
        this.errorSpanId = this.validationMsgId + new Date() +'-error-msg';
        this.statutsChangeSubscription = this.control.statusChanges.subscribe(
            (status)=>{
                if(status == 'INVALID'){
                    this.showError();
                }else{
    ...
    ngOnDestroy(): void {
        this.statutsChangeSubscription.unsubscribe();       
    }
    @HostListener('blur', ["$event"])
    handleBlurEvent(event){
        //This is needed to handel the case of clicking a required field and moving out.
        //Rest all are handled by status change subscription
        if(this.control.value == null || this.control.value == ''){
            if(this.control.errors) this.showError();
            else this.removeError();
